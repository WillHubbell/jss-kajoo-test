...
* `jss deploy config` to deploy the Sitecore config patch files to the Sitecore instance (you may need to add the `hostName` to your `hosts` file)
* Use `jss deploy app -c -d` to deploy the sample to Sitecore
* Visit `http://JssReactWeb` to see the demonstration running in Sitecore
